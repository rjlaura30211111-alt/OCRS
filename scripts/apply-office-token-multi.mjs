/**
 * Apply multi-token schema + seed first-generation PDF tokens.
 * Usage: node scripts/apply-office-token-multi.mjs
 */
...
const FIRST_BATCH_TOKENS = [
  ...
  ["ORLRDD", "orlddd_f2dd6d00c87b3226bd2bee96f4bb31e7a49d271dc63ae3f6"],
...
async function tokenExists(accessToken) {
...
for (const [office_code, access_token] of FIRST_BATCH_TOKENS) {
  if (await tokenExists(access_token)) {
    skipped += 1;
    continue;
  }

  const { error } = await supabase.from("office_access_tokens").insert({
    office_code,
    access_token,
  });
  ...
  if (error) {
    ...
    if (/office_access_tokens_pkey|primary key/i.test(error.message)) {
      console.error(
        "Cannot insert second token for the same office yet.\n" +
          "Run supabase/migrations/20260629160000_office_tokens_allow_multiple.sql in Supabase SQL Editor first."
      );
      process.exit(1);
    }

    console.error(`Failed to insert ${office_code}:`, error.message);
    process.exit(1);
  }

  inserted += 1;
  console.log(`Added first-batch token for ${office_code}`);
}
